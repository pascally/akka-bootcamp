            ...
            Receive<Watch>(watch =>
            {
                if (!_counterActors.ContainsKey(watch.Counter))
                {
                    ...
                    var counterActor = Context.ActorOf(Props.Create(() =>
                        new PerformanceCounterActor(watch.Counter.ToString(), //type
                                CounterGenerators[watch.Counter]))); //func de création

                    // add this counter actor to our index
                    _counterActors[watch.Counter] = counterActor; //association du PerformanceCounterActor au type
                }
